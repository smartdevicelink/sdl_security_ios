...
/// Manager class for downloading and caching a PFX file (A PFX file contains the certificate and public and private keys).
...
/// Initializes the certificate manager with the URL where the PFX files are hosted.
/// @param url The URL where the PFX files are being hosted.
- (instancetype)initWithCertificateServerURL:(NSString *)url;

/// Retrieves the unique PFX file associated with the appId. To retrieve the PFX file, the appId is appended to the query string of the url like so: `?appId=<appID parameter>`. The PFX data is then cached on disk.
